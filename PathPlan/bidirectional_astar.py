...
class BidirectionalAStarPlanner:
    ...
    def calc_final_bidirectional_path(self, n1, n2, set_a, set_b):
        rx_a, ry_a = self.calc_final_path(n1, set_a)
        rx_b, ry_b = self.calc_final_path(n2, set_b)
    ...
    def check_nodes_and_sets(self, c_nodes, closed_set_a, closed_set_b, n_ids):
        continue_ = [False, False]
        if not self.verify_node(c_nodes[0]) or n_ids[0] in closed_set_a:
            continue_[0] = True

        if not self.verify_node(c_nodes[1]) or n_ids[1] in closed_set_b:
            continue_[1] = True
...
class PathPlanner:
    ...
    def plan_path(self):
        planner = BidirectionalAStarPlanner(self.map.ox, self.map.oy, self.map.grid_size, self.map.robot_radius)
...
def plan():
    ...
    print("Bidirectional Astar plan start!!")
